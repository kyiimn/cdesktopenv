...
static void Redisplay (Widget gw, XEvent *event, Region region)
{
	...
	{
	  /* NOTE: Create the GC based on resources in the parent.  This
	     code assumes that the panner is the same color as the parent,
	     because this was quick to implement.  DJB */
	  XGCValues values;
	  XtGCMask  valueMask;
	  Arg args[3];

	  XtSetArg (args[0], XmNbottomShadowColor, &values.foreground);
	  ...
	  bottom_shadow_gc = XtGetGC ((Widget) pw, valueMask, &values);

	  XtSetArg (args[0], XmNtopShadowColor, &values.foreground);
      ...
      _XmDrawShadow (dpy, w, top_shadow_gc, bottom_shadow_gc,
		     2, kx, ky,
		     pw->panner.knob_width, pw->panner.knob_height);
    }
 /*
